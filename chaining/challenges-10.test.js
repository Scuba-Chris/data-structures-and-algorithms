...
Write a function named count that, given an integer and an array of arrays, uses either filter, map, or reduce to count the amount of times the
integer is present in the array of arrays.
...
const count = (target, input) => {
  // Solution code here...
  const reduced = input.reduce((accumlator, valueArr) => {
    accumlator += valueArr.filter(value => {
      return value === target;
    }).length;
    return accumlator;
  }, 0);
...
const totalSum = (input) => {
  // Solution code here...
  const total = input.reduce((accumlator, valueArr) => {
    accumlator += valueArr.reduce((accumlator) => {
      return accumlator++;
    })
    return accumlator;
  })
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here..
  const filterMap = input.map(arr => {
    return arr.filter(value => {
      return !isNaN(value) && Number(value) === value && value % 5 === 0;
    }).map(value => {
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  const charArr = data.filter(value => {
...
Write a function named findShortest that, given the Star Wars data from Challenge 6, uses any combination of filter, map and reduce to return the
name of the shortest character.
------------------------------------------------------------------------------------------------ */

let findShortest = (data) => {
  // Solution code here...
  const mappedStr = data.reduce((accumlator, value) => {
    if (Number(accumlator.height) > Number(value.height)) {
      accumlator = value;
    }
    return accumlator;
  })
